...
int producer_consumer(void)
{
    ...
    else
    {
        ...
        /* 创建消费者线程 */
    consumer_tid1 = rt_thread_create("consumer2",
                                    consumer2_thread_entry, RT_NULL,
                                    THREAD_STACK_SIZE,
                                    THREAD_PRIORITY + 1, THREAD_TIMESLICE);
    if (consumer_tid1 != RT_NULL)
    {
        rt_thread_startup(consumer_tid1);
    }
    else
    {
        rt_kprintf("create thread  consumer 2 failed");
        return -1;
    }
    return 0;
}
